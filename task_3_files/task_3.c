...
int main(int argc, char *argv[]) {
    ...
    matrix       = (double*)malloc(pow((N-1),2)*sizeof(double));
    all_vectors  = (double**)malloc((N-1)*sizeof(double*));
    gramm_matrix = (double*)malloc(pow((N-1),2)*sizeof(double));
    ...
    for (k=1; k<N; k++) {
        ...
        lambda = 4*pow(sin(M_PI*(k-1)/(2*N-2)),2)/pow(h,2);
        for (i=0; i<N-1; i++) {
        ...
        diff_normed = norm(result_of_mult, N-1)/((fabs(lambda) > 0) ? lambda : 1);
